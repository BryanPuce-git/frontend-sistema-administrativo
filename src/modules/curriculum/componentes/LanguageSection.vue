  ...
  <div class="mb-4">
    ...
    <div v-if="idiomasActive" class="p-4 border border-t-0 rounded-b-lg bg-gray-50">
      ...
      <div v-if="idiomas.length > 0" class="mb-4">
        <div v-for="idioma in idiomas" :key="idioma.idi_id" class="grid grid-cols-3 gap-4 items-center mb-2">
          <!-- Selección de idioma -->
          <div>
          ...
          <!-- Checkbox para excluyente -->
          <div>
            <label class="inline-flex items-center">
              <input type="checkbox" v-model="idioma.excluyente" class="form-checkbox text-red-500"
                @change="actualizarIdioma(idioma)" />
              <span class="ml-2">Excluyente</span>
            </label>
          </div>
        </div>
...
const obtenerIdiomas = async () => {
  try {
    ...
    idiomas.value = response.data.map(item => ({
      ...
      excluyente: item.idi_idioma_excluyente === 1, // Correcto
    }));
    console.log("Idiomas obtenidos:", idiomas.value);
  } catch (error) {
...
const agregarIdioma = async () => {
  if (!nuevoIdioma.value.idioma || !nuevoIdioma.value.nivel) {
    console.warn("Por favor selecciona un idioma y un nivel.");
    return;
  }
  const data = {
    pcom_id: props.id,
    ...
    idi_idioma_excluyente: excluyente.value ? 1 : 0,
  };
  try {
...
const actualizarIdioma = async (idioma) => {
  try {
    const data = {
      ...
      idi_idioma_excluyente: idioma.excluyente ? 1 : 0, // Incluye el campo excluyente
    };
    await useApi.patch(`/api/v1/curriculum/idioma/${idioma.idi_id}`, data);
    console.log(`Idioma con ID ${idioma.idi_id} actualizado correctamente.`);
  } catch (error) {
...
const eliminarIdioma = async (idi_id) => {
  try {
    ...
    console.log(`Idioma con ID ${idi_id} eliminado correctamente.`);
  } catch (error) {
